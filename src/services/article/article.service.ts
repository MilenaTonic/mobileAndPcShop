...
export class ArticleService extends TypeOrmCrudService<Article> {
    constructor(
        ...
        private readonly article: Repository<Article>, // Ovo navodimo u app.module.ts gde treba
    ...
    async createFullArticle(data: AddArticleDto): Promise<Article | ApiResponse> {
        ...
        for(let feature of data.features) {
            ...
            newArticleFeature.column_4 = feature.value;
    ...
    async editFullArticle(articleId: number, data: EditArticleDto ): Promise<Article | ApiResponse> {
        ...
        if(newPriceString !== lastPriceString) {
            ...
            // Upisivanje u article price repozitorijum
    ...
    async search(data: ArticleSearchDto): Promise<Article[]> {
        ...
        builder.innerJoinAndSelect(
            ...
            // OVO NIJE NAJBOLJI PRIMER ZA PRAKSU
        ...
        if (data.features && data.features.length > 0) {
            for (const feature of data.features) {
                builder.andWhere(
                    'af.featureId = :fId AND af.column_4 IN (:fVals)',
                    {
