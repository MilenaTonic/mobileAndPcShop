...
import { JwtRefreshDataDto } from "src/dtos/auth/jwt.refrest.dto";
...
export class AuthController {
    ...
    async doUserLogin(@Body() data: LoginUserDto, @Req() req: Request): Promise<LoginInfoDto | ApiResponse> {
        ...
        const jwtRefreshDataDto = new JwtRefreshDataDto();
        jwtRefreshDataDto.role = jwtData.role;
        jwtRefreshDataDto.id = jwtData.id;
        jwtRefreshDataDto.identity = jwtData.identity;
        jwtRefreshDataDto.exp = this.getDatePlus(60 * 60 * 24 * 31);
        jwtRefreshDataDto.ip = jwtData.ip;
        jwtRefreshDataDto.ua = jwtData.ua;

        let refreshToken: string = jwt.sign(jwtRefreshDataDto.toPlainObject(), jwtSecret); // Refresh token
        ...
        const responseObject = new LoginInfoDto(
            ...
            token,
            refreshToken,
            this.getIsoDate(jwtRefreshDataDto.exp),
        );

        await this.userService.addToken(
            user.userId,
            refreshToken, 
            this.getDatebaseDateFormat(this.getIsoDate(jwtRefreshDataDto.exp))
        );
